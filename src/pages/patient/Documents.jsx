...
const CATEGORIES = [
  { id: 'recetas',       name: 'Recetas Digitales', icon: FileText,   color: '#2563EB', bg: '#EFF6FF', uploadable: false },
...
export default function PatientDocuments({ profile }) {
  ...
  if (viewingCat) {
    const catDocs = docs[viewingCat.id] || []
    ...
    return (
      <div className="absolute inset-0 bg-[#F8FAFC] flex flex-col animate-fade-in">
        ...
        <div className="flex-1 overflow-y-auto p-6 pb-32 scrollbar-hide space-y-6 bg-[#F8FAFC]">
          ...
          {viewingCat.uploadable && (
            <div>
              <div className="flex justify-between items-center mb-3">
                <h3 className="text-[12px] font-bold text-gray-500 uppercase tracking-widest flex items-center gap-2">
                  Mis Controles
                </h3>
                <button
                  onClick={() => { setUploadCat(viewingCat); setNewDocName(''); setShowUpload(true) }}
                  className="text-[11px] font-bold text-brand bg-blue-50 px-3 py-1.5 rounded-full hover:bg-blue-100 flex items-center gap-1 border border-blue-100"
                >
              ...
              <div className="space-y-3">
                {patDocs.length > 0 ? patDocs.map(doc => (
                  <div key={doc.id} className="bg-white p-4 rounded-[24px] shadow-sm flex justify-between items-center border border-gray-100">
                    <div className="flex items-center gap-4">
                      <div className="w-12 h-12 bg-[#F8FAFC] border border-gray-100 rounded-[14px] flex items-center justify-center">
  ...
  return (
    <div className="absolute inset-0 bg-[#F8FAFC] pt-6 sm:pt-8 pb-32 px-6 overflow-y-auto animate-fade-in scrollbar-hide">
      ...
      {/* Upload modal */}
      {showUpload && uploadCat && (
        <div className="absolute inset-0 bg-gray-900/50 backdrop-blur-sm z-[80] flex flex-col justify-end animate-fade-in">
          <div className="w-full bg-white rounded-t-[40px] shadow-[0_-20px_50px_rgba(0,0,0,0.2)] pb-12 pt-4 px-6 animate-slide-up-spring border-t border-gray-100">
            ...
            <div className="bg-[#F8FAFC] rounded-[24px] p-6 shadow-sm border border-gray-100 mb-6">
              ...
              <div className="border-2 border-dashed border-blue-200 rounded-[20px] p-8 flex flex-col items-center justify-center bg-blue-50/50 cursor-pointer hover:bg-blue-50 transition-colors">
                <Camera className="w-10 h-10 text-blue-400 mb-3" />
                <p className="font-bold text-[14px] text-blue-800">Tocá para seleccionar archivo</p>
              </div>
            </div>
            <button
