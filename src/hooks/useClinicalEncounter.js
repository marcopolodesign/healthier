...
export function useClinicalEncounter({ consultationId, patientId, professionalId, specialty, modality, licenseType, licenseNumber, preconsulta }) {
  ...
  // Promesa compartida entre llamadas concurrentes de ESTA instancia del hook.
  // Dos taps rápidos en la guía clínica (p.ej. marcar dos banderas rojas
  // seguidas) llamaban a `ensureEncounter()` antes de que la primera terminara
  // de crear el encuentro — `encounterId` todavía era null para las dos, así
  // que las dos creaban. El índice único (migración 076) hace fallar el
  // segundo INSERT, y esa nota se perdía con un toast de error. Ahora toda
  // llamada mientras hay una creación en vuelo espera la MISMA promesa.
  ...
  const ensureEncounter = useCallback(async () => {
    ...
    const promise = (async () => {
      try {
        ...
        let enc
        try {
          enc = await clinicalService.createEncounter({
            patientId,
            professionalId,
            consultationId,
          ...
          // Violación del índice único (23505, migración 076): otra instancia
          // de este hook (o la ventana de carrera que la promesa compartida de
          // arriba no llega a cubrir, porque son dos hooks distintos) ya creó
          // el encuentro entre el chequeo y este INSERT. No es un error real:
          // se relee y se sigue con ese en vez de perder la nota.
          if (err?.code === '23505' && consultationId) {
            const existente = await clinicalService.getEncounterByConsultationIdSafe(consultationId)
            if (existente) {
              setEncounterId(existente.id)
              return existente.id
            }
